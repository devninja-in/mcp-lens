...
The `mcp-lens` CLI lets you run evaluations headlessly — useful for CI pipelines and automation.

```bash
# Install (if you haven't already run `make setup`)
...
> **Tip:** If you activate the virtualenv (`source .venv/bin/activate`), you can drop the `uv run` prefix and use `mcp-lens` directly.
...
Bearer tokens and API keys are encrypted at rest (Fernet/AES-128-CBC) and stored in the local database. Set `MCP_LENS_ENCRYPTION_KEY` in `.env` to enable encryption (`make setup` generates this automatically). Without the key, secrets are stored in plaintext. OAuth tokens can optionally be persisted across sessions with `PERSIST_OAUTH_TOKENS=true`.
...
- Bearer tokens and API keys are encrypted at rest in the database when `MCP_LENS_ENCRYPTION_KEY` is set (auto-generated by `make setup`)
